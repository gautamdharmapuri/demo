...
<head><base href="/">
	...
	<!-- Basic Page Needs -->
	<meta charset="utf-8">
	<title><?php echo $defaultState;  ?> Electronics Ad View | NRIs</title>
...
<!-- Section-1 START-->	
        ...
        <?php
			...
			if(isset($_POST['comment_submit']) && $_POST['comment_submit'] != '') {
				...
				$query = mysql_query("INSERT INTO electronics_comments (post_id,comment,bid,commented_by)
			...
			$query="select a.*, b.name from post_free_electronics a, electronics b where a.AdsCat = b.id  and md5(a.id) = '".$_GET['ViewId']."'"; 
			$result=mysql_query($query);
			$rs=mysql_fetch_array($result);	
			$total_views = $rs['total_views'] + 1 ;
			mysql_query("update post_free_electronics set total_views='".$total_views."' where md5(id) = '".$_GET['ViewId']."'");
	...
	<h4><a href="<?php echo $siteUrlConstant;?>state?State=<?php echo $defaultState;?>" style="color:#0033FF;">Home</a> >> Electronics</h4>
				...
				<div class="col-md-3" >
					<?php
						...
						if($rs['image'.$tempVar] != '') { ?>
							<div class="col-sm-12">
								<a href="javascript:;">
									<img class="myImageClass" src="<?php echo 'uploads/electronics/'.$rs['image'.$tempVar];?>" style="width:100%;height:140px;" alt="<?php echo $rs['image'.$tempVar];?>">
								</a>
							</div>
					<?php } ?>
				</div>
...
<div id="googleMap" style="width:100%;height:300px;" ></div>
 ...
 <?php include "reply-widget.php"; ?>
			...
			<?php
				$query1 = "SELECT electronics_comments.*,CONCAT(register.fname,' ',register.lname) as username
							FROM electronics_comments,register
							WHERE post_id = ".$rs['id']." AND electronics_comments.commented_by = register.id
							GROUP BY electronics_comments.id";
