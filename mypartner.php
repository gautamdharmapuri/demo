...
<body>
<h1 style="display: none;">Find The Right Partner For You</h1>
...
<?php } ?>    
		...
		{		
			$query  = "select a.*, b.name from post_free_mypart a, my_partner b where a.EndDate >= '".$current_date."' and a.Category = b.id and a.States  IN ('ALL','multiple')   order by a.total_views desc";
						...
						{ ?> 
					<tr>                                                                             
					<td><a href="<?php echo $siteUrlConstant;?>mypartner_view?ViewId=<?php echo md5($rs['id']);?>"  onMouseMove="this.style.color='red'" onMouseOut="this.style.color='black'"><?php echo ucwords($rs['TitleAD']);?></a></td>
					<td><a href="<?php echo $siteUrlConstant;?>mypartner_view?ViewId=<?php echo md5($rs['id']);?>"  onMouseMove="this.style.color='red'" onMouseOut="this.style.color='black'"><?php echo ucwords($rs['name']);?></a></td>
					
					<td><a href="<?php echo $siteUrlConstant;?>mypartner_view?ViewId=<?php echo md5($rs['id']);?>"  onMouseMove="this.style.color='red'" onMouseOut="this.style.color='black'"><?php  echo $rs['total_views'];?></a></td>
					</tr>
					<?php }  } else {  ?>
