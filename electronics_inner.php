...
<div class="widget-temple">
	...
	<h4><a href="state.php?State=<?php echo $state;?>" style="color:#0033FF;">Home</a> >> Electronics</h4>
 <?php
if(isset($_SESSION['Nris_session']))	  
{ ?>
<a href="electronics_create.php?code=<?php echo $_SESSION['state'] ?>&type=premium"  class="btn btn-default" style="background-color:#0000FF;color:#FFFFFF;float:right;">Create Premium Post <img src="images/arrow.gif"></a>    
<a href="electronics_create.php?code=<?php echo $_SESSION['state'] ?>"  class="btn btn-default" style="background-color:#990033;color:#FFFFFF;float:right;">Create Free Post <img src="images/arrow.gif"></a>    
  <?php } else { ?> 
...
<table align="center" >
			...
			//		$query = "select a.*, b.name, c.model_name from post_free_electronics a, auto_makes b, auto_models c where a.Brand = b.id and a.SubBrand=c.id  and  a.States  IN ('".$_SESSION['state']."','ALL')  order by a.total_views desc";	
			
			//		$query = "select a.*, b.name, c.model_name from post_free_electronics a, auto_makes b, auto_models c where a.Brand = b.id and a.SubBrand=c.id  and   FIND_IN_SET('".$_SESSION['state']."',  a.States_Details)  order by a.total_views desc";	
			
	//				$query = "select a.*, b.name, c.model_name from post_free_electronics a, auto_makes b, auto_models c where a.Brand = b.id and a.SubBrand=c.id  and  a.States  IN ('".$_SESSION['state']."','ALL') and   FIND_IN_SET('".$_SESSION['state']."',  a.States_Details)  order by a.total_views desc";	
	
	
		//			$query = "select a.*, b.name, c.model_name from post_free_electronics a, auto_makes b, auto_models c where a.Brand = b.id and a.SubBrand=c.id  and  a.States  IN ('".$_SESSION['state']."','ALL') or (  select a.*, b.name, c.model_name from post_free_electronics a, auto_makes b, auto_models c where a.Brand = b.id and a.SubBrand=c.id  and   FIND_IN_SET('".$_SESSION['state']."',  a.States_Details)  order by a.total_views desc)";	
		...
		if(isset($_POST['cmdsubmit']))
		{
			$cat = $_POST['AdsCat'];
//			echo $cat;
			$query = "select a.*, b.name from post_free_electronics a, electronics b where a.AdsCat= '".$cat."' and '".$cat."' = b.id and EndDate> now() and States IN ('".$state."','All') order by a.total_views desc";			
		}
		else
		{		
			$query = "select a.*, b.name from post_free_electronics a, electronics b where a.AdsCat = b.id and EndDate> now() and States IN('".$state."','All') order by a.total_views desc";			
		}
                                                                                ...
                                                                                { ?> 
                                                <tr>                                                                             
                                                <td><a href="electronics_inner_view.php?ViewId=<?php echo md5($rs['id']);?>"  onMouseMove="this.style.color='red'" onMouseOut="this.style.color='black'"><?php echo ucwords($rs['TitleAD']);?></a></td>
