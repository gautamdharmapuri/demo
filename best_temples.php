...
//	$query = "SELECT COUNT(*) as num FROM $tableName order by total_views desc";
	$query = "SELECT DISTINCT  * FROM fam_temples LEFT OUTER JOIN rating_temple ON fam_temples.id = rating_temple.temple_id GROUP BY(fam_temples.id) order by fam_temples.total_views desc, rating_temple.rate desc limit 25";
...
//	echo $query1;
	...
	{	
		...
		$paginate.= "</div>";		
				...
				?> 
                            <tr>
                                <td style="padding:5px;"><img src="admin/uploads/temples/<?php echo $rs['image'];?>" style="height:50px;width:50px;border-radius: 50%;" alt="<?php echo ucwords($rs['temple_name']);?>"></td>
            ...
            echo ucwords($rcity['city']);  ?>
                                ...
                                Comments : 
                                <?php
								$resultcmt=mysql_query("SELECT count(*) as total from comment_temple where temple_id ='".$rs[0]."'");
