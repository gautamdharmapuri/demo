            ...
            <div class="col-md-12" style="text-align:left;color:#000000;"> 
                       ...
                       <br><h4 class="myheadline4"><a href="<?php echo $siteUrlConstant;?>" style="color:#0033FF;">Home</a> >> Casinos >> <?php 
					  	 $query="select a.*, b.state from fam_casinos a, states b where a.state_code = b.state_code and md5(a.id) = '".$_GET['ViewId']."'";							
									...
									mysql_query("update fam_casinos set total_views='".$total_views."' where md5(id) = '".$_GET['ViewId']."'");
					    ?></h4>
...
<p class="mydata" align="center" style="text-align:center;">
        ...
        <img src="admin/uploads/casinos/<?php echo $rs['image'];?>" width="80%" style="height:400px !important;" class="imgframe" alt="<?php echo ucwords($rs['name']);?>"> 	<?php }  else {  ?>
        <img src="admin/img/no_image.png" height="auto" width="300" class="imgframe" alt="<?php echo ucwords($rs['name']);?>">
        <?php } ?>
...
<p class="mydata"><b>Casinos  :</b> <a href="https://maps.google.com/maps?saddr=&daddr=<?php echo urlencode($rs['address']);?>" target="_blank" class="address_link">
...
$date=date('Y-m-d');
$query_rate_before = mysql_query("select * from rating_casinos where login_id='".$myid."' and casino_id ='".$pro_id."' ");
...
{
 $query_rate = "insert into rating_casinos(casino_id,rate,login_id,login_name,date) values('".$pro_id."','".$star."','".$myid."','".$new_name."','".$date."')";		 
 $result_rate = mysql_query($query_rate);
}
...
<?php 
$star1 = "SELECT COUNT(id) AS star1Id FROM  rating_casinos WHERE casino_id = '".$rs['id']."' and rate='1'";
$result_start1 = mysql_query($star1);
$star_fs1 = mysql_fetch_array($result_start1);

$star2 = "SELECT COUNT(id) AS star2Id FROM  rating_casinos WHERE casino_id = '".$rs['id']."' and rate='2'";
$result_start2 = mysql_query($star2);
$star_fs2 = mysql_fetch_array($result_start2);

$star3 = "SELECT COUNT(id) AS star3Id FROM  rating_casinos WHERE casino_id = '".$rs['id']."' and rate='3'";
$result_start3 = mysql_query($star3);
$star_fs3 = mysql_fetch_array($result_start3);

$star4 = "SELECT COUNT(id) AS star4Id FROM  rating_casinos WHERE casino_id = '".$rs['id']."' and rate='4'";
$result_start4 = mysql_query($star4);
$star_fs4 = mysql_fetch_array($result_start4);

$star5 = "SELECT COUNT(id) AS star5Id FROM  rating_casinos WHERE casino_id = '".$rs['id']."' and rate='5'";
...
<?php
$tmpl_cmnt = "select a.*, b.* from  comment_casinos a, register b where a.member_id = b.id  and md5(a.casino_id) = '".$_SESSION['ViewId']."' order by a.cmnt_id desc" ;
...
if(isset($_POST['cmdcomment']))	
{
	...
	$query_cmt = "insert into comment_casinos(casino_id,member_id,comment,cmnt_date,cmnt_time) values('".$TMPId."','".$mId."','".$a."','".$date."','".$time."')";		 
