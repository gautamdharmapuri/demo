...
<head><base href="/">
	...
	<!-- Basic Page Needs -->
	<meta charset="utf-8">
	<title>Restaurants View | NRIs</title>
...
<!-- Section-1 START-->	
        ...
        <div class="col-md-8 inner-middle-wrap">
            ...
            <div class="col-md-12" style="text-align:left;color:#000000;"> 
                       ...
                       <br><h4 class="myheadline4"><a href="<?php echo $siteUrlConstant;?>" style="color:#0033FF;">Home</a> >> Restaurants >> <?php 
					  	 $query="select a.*, b.state from fam_restaurants a, states b where a.state_code = b.state_code and md5(a.id) = '".$_GET['ViewId']."'";							
                                    $result=mysql_query($query);
                                    $rs=mysql_fetch_array($result);
									echo ucwords($rs['rest_name']); 
									$total_views = $rs['total_views'] + 1 ;
									mysql_query("update fam_restaurants set total_views='".$total_views."' where md5(id) = '".$_GET['ViewId']."'");
					    ?></h4>
...
<p class="mydata" align="center" style="text-align:center;">
        ...
        <img alt="<?php echo ucwords($rs['rest_name']); ?>" src="admin/uploads/restaurants/<?php echo $rs['image'];?>" width="80%" style="height:400px !important;"  class="imgframe"> 	<?php }  else {  ?>
        <img alt="<?php echo ucwords($rs['rest_name']); ?>" src="admin/img/no_image.png" height="auto" width="300" class="imgframe">
        <?php } ?>
...
$date=date('Y-m-d');
$query_rate_before = mysql_query("select * from rating_restaurant where login_id='".$myid."' and res_id ='".$pro_id."' ");

if(mysql_num_rows($query_rate_before)>0)
{
		 $query_rate = "update rating_restaurant set res_id = '".$pro_id."', rate='".$star."', login_id = '".$myid."', login_name = '".$new_name."', date = '".$date."' where login_id='".$myid."' and res_id ='".$pro_id."'";		 
...
<?php 
$star1 = "SELECT COUNT(id) AS star1Id FROM  rating_restaurant WHERE res_id = '".$rs['id']."' and rate='1'";
$result_start1 = mysql_query($star1);
$star_fs1 = mysql_fetch_array($result_start1);

$star2 = "SELECT COUNT(id) AS star2Id FROM  rating_restaurant WHERE res_id = '".$rs['id']."' and rate='2'";
$result_start2 = mysql_query($star2);
$star_fs2 = mysql_fetch_array($result_start2);

$star3 = "SELECT COUNT(id) AS star3Id FROM  rating_restaurant WHERE res_id = '".$rs['id']."' and rate='3'";
$result_start3 = mysql_query($star3);
$star_fs3 = mysql_fetch_array($result_start3);

$star4 = "SELECT COUNT(id) AS star4Id FROM  rating_restaurant WHERE res_id = '".$rs['id']."' and rate='4'";
$result_start4 = mysql_query($star4);
$star_fs4 = mysql_fetch_array($result_start4);

$star5 = "SELECT COUNT(id) AS star5Id FROM  rating_restaurant WHERE res_id = '".$rs['id']."' and rate='5'";
...
<?php
$tmpl_cmnt = "select a.*, b.* from  comment_restaurant a, register b where a.member_id = b.id  and md5(a.res_id) = '".$_SESSION['ViewId']."' order by a.cmnt_id desc" ;
...
if(isset($_POST['cmdcomment']))	
{
	...
	$query_cmt = "insert into comment_restaurant(res_id,member_id,comment,cmnt_date,cmnt_time) values('".$TMPId."','".$mId."','".$a."','".$date."','".$time."')";		 
