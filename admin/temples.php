...
<?php  if (isset($_GET['delId']))
		...
		$qy="delete from fam_temples where id='".$_GET['delId']."'";
		mysql_query($qy);
		
	
		echo "<script language='javascript' type='text/javascript'>alert('Temple Deleted Successfully...');</script>";
...
<script language="javascript" type="text/javascript">
	function delthis()
	{	
		var ans=confirm("Are you sure You want to Delete this Temple Details\?");
		if (ans==true)
		{
			return true;
		}
               ...
               <!-- START X-NAVIGATION VERTICAL -->
                                ...
                                <div class="col-md-2 col-xs-12" style="float:right">
												<script type="text/javascript">
													  function change_state() {
															...
															if (sel != '') {
																  window.location.href = admin_url+'temples.php?state='+sel;    
                                                            } else {
																  window.location.href = admin_url+'temples.php';
															}
                                            ...
                                            <div class="input-group">
                                                <select class="form-control" name="state_filter" onchange="change_state();">
													  ...
													  <?php
															$query1 = "SELECT a.state_code,s.state,count(a.id) as cnt FROM fam_temples a,states s
															WHERE a.state_code = s.state_code AND a.state_code != ''
															GROUP BY a.state_code ORDER BY a.state_code ASC";
				...
				<tbody>
					  <?php
												...
												if($noofstates==0) 
												{											   
											    	$query="select a.*, b.state from fam_temples a, states b where a.state_code = b.state_code $where order by a.id desc";
												}
												if($noofstates==1)
												{
													//$query="select * from  states where state_code = ".$join_states_list." order by state";
													$query="select a.*, b.state from fam_temples a, states b where a.state_code = ".$join_states_list." and  a.state_code = b.state_code $where order by a.id desc";
												} 
												
												if($noofstates>1)
												{
													//$query="select * from  states where state_code in(".$join_states_list.") order by state";
													$query="select  a.*, b.state from fam_temples a, states b where a.state_code in(".$join_states_list.") and a.state_code = b.state_code $where order by a.id desc";
                                         ...
                                         <tr>
                                            ...
                                            {  ?> 
                                            &nbsp;
                                            <a class="btn btn-danger" style="background-color:#FF0000;border:#0066CC;padding:4px 10px;" title="Delete" href="temples.php?delId=<?php echo $rs['id'];?>" onClick="return delthis();" >
