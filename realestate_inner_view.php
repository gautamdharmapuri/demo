...
<head><base href="/">
	...
	<!-- Basic Page Needs -->
	<meta charset="utf-8">
	<title><?php echo $defaultState;  ?> Real Estate Ad View | NRIs</title>
...
<!-- Section-1 START-->	
        ...
        <?php
			$query="select a.*, b.name from post_free_real_estate a,  real_estate b where a.AdsCat = b.id and md5(a.id) = '".$_GET['ViewId']."'"; 
			$result=mysql_query($query);
			$rs=mysql_fetch_array($result);	
			$total_views = $rs['total_views'] + 1 ;
			mysql_query("update post_free_real_estate set total_views='".$total_views."' where md5(id) = '".$_GET['ViewId']."'");
...
<h4><a href="<?php echo $siteUrlConstant;?>state?State=<?php echo $defaultState;?>" style="color:#0033FF;">Home</a> >> Real Estate</h4>
  <?php
				...
				<div class="col-md-3" >
					<?php
						...
						if($rs['image'.$tempVar] != '') { ?>
							<div class="col-sm-12">
								<a href="javascript:;">
									<img class="myImageClass" src="<?php echo 'uploads/real_estate/'.$rs['image'.$tempVar];?>" style="width:100%;height:140px;" alt="<?php echo $rs['image'.$tempVar];?>">
								</a>
							</div>
					<?php } ?>
				</div>
